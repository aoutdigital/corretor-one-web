...
export async function GET(request: Request) {
  ...
  if (!placeId) {
    return NextResponse.json(
      { ok: false, error: { code: "VALIDATION_ERROR", message: "placeId é obrigatório" } },
      { status: 400 },
    );
  }
  ...
  if (!response.ok) {
    return NextResponse.json(
      { ok: false, error: { code: "EXTERNAL_API_ERROR", message: `Google Place Details retornou ${response.status}` } },
      { status: 502 },
    );
  }
  ...
  const cidade =
    ...
    "";
  const estado = findComponent(components, "administrative_area_level_1")?.short_name ?? "";
  ...
  return NextResponse.json({
    ...
    data: {
      ...
      logradouro: route,
      numero: findComponent(components, "street_number")?.long_name ?? "",
      bairro,
      cidade,
      estado: estado.toUpperCase(),
      cep: findComponent(components, "postal_code")?.long_name ?? "",
      lat: payload.result.geometry?.location?.lat ?? null,
      lng: payload.result.geometry?.location?.lng ?? null,
