...
export async function GET(request: Request, { params }: Params) {
  ...
  const result = await listMidiaEmpreendimento(accessToken, id);
  if (!result.ok) {
...
export async function POST(request: Request, { params }: Params) {
  ...
  const midiaIdValue = form.get("midia_id");
  const hasYoutubeUrl = typeof youtubeUrlValue === "string" && youtubeUrlValue.trim().length > 0;
  const hasMidiaId = typeof midiaIdValue === "string" && midiaIdValue.trim().length > 0;

  if (!(file instanceof File) && !hasYoutubeUrl && !hasMidiaId) {
    return NextResponse.json(
      {
        ok: false,
        error: {
          code: "VALIDATION_ERROR",
          message: "file, youtube_url or midia_id is required",
        },
      },
      { status: 400 },
    );
  ...
  if (hasMidiaId) {
    const result = await attachExistingMidiaEmpreendimento(accessToken, id, {
      midiaId: (midiaIdValue as string).trim(),
      ordem,
      titulo: typeof tituloValue === "string" && tituloValue.length > 0 ? tituloValue : null,
      alt: typeof altValue === "string" && altValue.length > 0 ? altValue : null,
      legenda: typeof legendaValue === "string" && legendaValue.length > 0 ? legendaValue : null,
  ...
  const result = await createYoutubeMidiaEmpreendimento(
    accessToken,
    id,
...
export async function PATCH(request: Request, { params }: Params) {
  ...
  const result = await reorderMidiaEmpreendimento(accessToken, id, orderedMidiaIds);
  if (!result.ok) {
    return NextResponse.json(result, {
      status: statusFromErrorCode(result.error.code),
    });
