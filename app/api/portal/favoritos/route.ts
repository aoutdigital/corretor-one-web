...
export async function POST(request: Request) {
  ...
  const imovelId = (body as Record<string, unknown>)?.imovel_id;
  if (typeof imovelId !== "string" || !imovelId) {
    return NextResponse.json(
      { ok: false, error: { code: "VALIDATION_ERROR", message: "imovel_id is required" } },
      { status: 400 },
    );
  }
  ...
  const result = await db
    .from("user_favoritos")
    .insert({ user_id: auth.data.userId, imovel_id: imovelId })
    .select("id")
    .single();
