...
export async function POST(request: Request) {
  ...
  const corretorId = (body as Record<string, unknown>)?.corretor_id;
  if (typeof corretorId !== "string" || !corretorId) {
    return NextResponse.json(
      { ok: false, error: { code: "VALIDATION_ERROR", message: "corretor_id is required" } },
      { status: 400 },
    );
  }
  ...
  const result = await db
    .from("user_follows")
    .insert({ user_id: auth.data.userId, corretor_id: corretorId })
    .select("id")
    .single();
