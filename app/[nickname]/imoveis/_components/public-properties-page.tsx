...
import { ArrowRight, Bathtub, Bed, Buildings, Car, FunnelSimple, HouseLine, MagnifyingGlass, MapPin, Ruler } from "@phosphor-icons/react/dist/ssr";
            ...
            <div className="min-w-0">
              ...
              {imoveis.length > 0 ? (
                <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
                  {imoveis.map((imovel) => (
                    <PropertyCard key={imovel.id} nickname={profile.nickname ?? nickname} imovel={imovel} imageUrl={mediaById.get(imovel.id) ?? null} />
                  ))}
                </div>
              ) : (
...
function PropertyCard({ nickname, imovel, imageUrl }: { nickname: string; imovel: ImovelRow; imageUrl: string | null }) {
  const title = buildImovelHeaderTitle(imovel);
  const operation = imovel.tipo_negociacao === "ALUGUEL" ? "aluguel" : "venda";
  const price = imovel.tipo_negociacao === "ALUGUEL" && imovel.preco_locacao ? imovel.preco_locacao : imovel.preco_venda;
  ...
  const stats = [
    ...
    imovel.dormitorios ? { icon: Bed, label: `${padSmallNumber(imovel.dormitorios)} ${imovel.dormitorios === 1 ? "dormitório" : "dormitórios"}` } : null,
    imovel.suites ? { icon: Bed, label: `${padSmallNumber(imovel.suites)} ${imovel.suites === 1 ? "suíte" : "suítes"}` } : null,
    imovel.banheiros ? { icon: Bathtub, label: `${padSmallNumber(imovel.banheiros)} ${imovel.banheiros === 1 ? "banheiro" : "banheiros"}` } : null,
    imovel.vagas ? { icon: Car, label: `${padSmallNumber(imovel.vagas)} ${imovel.vagas === 1 ? "vaga" : "vagas"}` } : null,
  ...
  return (
    ...
    >
      <div className="relative aspect-[4/3] bg-stone-100">
        {imageUrl ? (
          <Image src={imageUrl} alt={title} fill sizes="(min-width: 1280px) 33vw, 100vw" className="object-cover transition duration-500 group-hover:scale-[1.03]" unoptimized />
        ) : (
      ...
      <div className="p-5">
        <p className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--grey-olive)]">{formatEnumLabel(imovel.tipo)}</p>
        ...
        <div className="mt-5 border-t border-stone-100 pt-4">
          ...
          {charges.length > 0 ? (
            <p className="mt-2 flex min-w-0 flex-nowrap items-center gap-2 overflow-hidden text-[11px] font-light leading-none text-slate-500">
              {charges.map((charge) => (
                <span key={charge} className="min-w-0 shrink truncate">{charge}</span>
              ))}
            </p>
          ) : null}
        </div>
...
function formatAreaSummary(imovel: ImovelRow) {
...
function buildAddressLine(imovel: ImovelRow) {
